# Hofino – Design-Brief für Claude Design (Mobile-First Redesign)

> **So benutzt du dieses Dokument:** Lege auf **claude.ai/design** ein **Design-System-Projekt**
> an, füge dieses Dokument als Auftrag ein und hänge die Screenshots aus dem Ordner `images/` an
> (zeigen den **aktuellen** Stand der App – das, was du überarbeitest). Claude Design baut daraus
> ein Design-System aus Komponenten-Previews + Screen-Mockups. Dieses Projekt wird anschließend
> per **DesignSync** zurück nach Claude Code gelesen und dort in React Native umgesetzt – baue das
> System deshalb sauber strukturiert (siehe §0 „Projektstruktur").

---

## 0. Auftrag an dich (Claude Design)

Du bist Senior Product- & UI-Designer für **Edu-Fintech-Apps**. Entwirf ein **modernes,
mobile-first** Design für die App **Hofino** und baue es als **Design-System-Projekt** auf.
...
2. Liefere **3 deutlich unterschiedliche Design-Richtungen** (nicht Varianten derselben Idee).
   ...
   - **Mockups für 3 Schlüssel-Screens** als gerenderte iPhone-Mockups (**390×844**):
     **(a) Kinder-Zuhause mit Haus**, **(b) Lernen**, **(c) Depot**.
...
3. **Ich wähle dann eine Richtung aus.** Danach detaillierst du diese eine Richtung für **alle**
   Screens aus §5 als **gerenderte Mockups (390×844)** und lieferst das vollständige
   **Design-System** (Tokens, Spacing-Skala, alle Komponenten als Preview-Cards).

**Projektstruktur (wichtig für den Rückweg via DesignSync):**
- Lege **Foundations** (Farb-Tokens, Typo-Skala, Spacing, Radien, Schatten) als eigene
  Preview-Cards an (Gruppe „Type"/„Colors"/„Spacing").
- Lege **jede Komponente** als eigene Preview-Card an (Gruppe „Components"): Card, Button
  (primär/sekundär/ghost), Pill/Badge, Progress-Bar, Tab-Bar, Input, Listenzeile, Belohnungs-Moment.
- Lege **jeden Screen** als eigene Mockup-Card an (Gruppe „Screens").
- Halte die **Token-Werte als CSS-Variablen** explizit im Code sichtbar (z. B. `--color-primary`),
  damit sie 1:1 in `theme.ts` übernommen werden können.
...
Backdrop-Blur, keine CSS-Grid-Tricks). Verläufe, Schatten, abgerundete Ecken, SVG-Illustrationen
sind ok. Die HTML-Previews sind **Referenz/Spec** – der Code wird in RN neu implementiert, nicht
1:1 übernommen.
...
**Design-Tokens (aktueller Stand, „Mission Board"):**
| Token | Hex | Bedeutung |
|---|---|---|
...
| softBlue / softMint | `#EAF2FB` / `#E4F8EF` | weiche Akzentflächen |

**Schriften (aktuell):** **Space Grotesk** (Display/Überschriften/Zahlen) + **Inter** (Fließtext).
Karten-Radius 22, weicher Schatten; Button-Radius 16. Tab-Bar mit Outline-Icons (keine Emojis).

**Logo:** stilisiertes **„H" mit Wachstumslinie** (aktuell: grünes H auf Navy,
...
- [ ] Warten auf meine Auswahl → dann **vollständiges Design-System-Projekt** der gewählten
      Richtung, strukturiert nach §0 „Projektstruktur":
      - Foundations als Cards (Colors/Type/Spacing/Radien/Schatten, Token-Werte als CSS-Variablen)
      - alle Komponenten als Preview-Cards (Card, Buttons, Pill/Badge, Progress, Tab-Bar, Input,
        Listenzeile, Belohnungs-Moment)
      - alle 9 Screens (§5) als Mockup-Cards (390×844)
      - Haus-System (§6): 6 Stufen × 3 Grundstücke + Auszeichnungs-Objekte als Illu-Spec

---

## Anhang A – Aktueller Stand (Screenshots)

Die folgenden Bilder zeigen den **aktuellen** Stand (Navy/Gold-Tokens + Space Grotesk/Inter,
Mission-Board-Layout) – das ist die Baseline, die du überarbeitest. (Bilder im Ordner `images/`.)
